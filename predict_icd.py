...
## Initiate web service application using Flask
app = Flask(__name__)
...
# Get input from the user and send it to the predict_icd function
def input():
    if request.method == "POST":
	    descript = request.form["descript"]
	    return redirect(url_for("predict_icd", description=descript))
    else:
...
def predict_icd(description):
    ...
    if description in compound_nouns:
        return ICD_codes[compound_nouns.index(description)]
